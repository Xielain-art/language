...
// Main Settings Menu - with submenus
export const settingsMenu = new Menu<Context>('settings-menu')
  .text(ctx => ctx.t('menu-settings-comm-submenu'), async (ctx) => {
    await ctx.editMessageText(ctx.t('menu-settings-communication-title'), { parse_mode: 'HTML' })
    ctx.menu.nav('settings-communication-menu')
  })
  .row()
  .text(ctx => ctx.t('menu-settings-edu-submenu'), async (ctx) => {
    await ctx.editMessageText(ctx.t('menu-settings-education-title'), { parse_mode: 'HTML' })
    ctx.menu.nav('settings-education-menu')
  })
  .row()
  .text(ctx => ctx.t('menu-settings-system-submenu'), async (ctx) => {
    await ctx.editMessageText(ctx.t('menu-settings-system-title'), { parse_mode: 'HTML' })
    ctx.menu.nav('settings-system-menu')
  })
  .row()
  .back(
...
// Communication Settings Submenu
export const settingsCommunicationMenu = new Menu<Context>('settings-communication-menu')
...
// Education Settings Submenu
export const settingsEducationMenu = new Menu<Context>('settings-education-menu')
...
export const settingsSystemMenu = new Menu<Context>('settings-system-menu')
